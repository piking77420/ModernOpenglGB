...
void SphereGizmo::Draw(Scene* scene, Shader* shader)
{
	...
	Matrix4X4 model = Matrix4X4().TRS(EntityAttachTo.transform.GetWordlPos(), EntityAttachTo.transform.rotate,Vector3(radius, radius, radius));
...
SphereGizmo::SphereGizmo(Entity& entityAttach,float& _radius) : GizmoRenderer(entityAttach) , radius(_radius)
{
	...
	m_Model = entityAttach.scene->ressourcesManagers->GetElement<Model>("Sphere.obj");
