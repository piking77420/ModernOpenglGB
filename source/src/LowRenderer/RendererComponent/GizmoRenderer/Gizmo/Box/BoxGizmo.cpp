...
void BoxGizmo::Draw(Scene* scene, Shader* shader)
{
	...
	Matrix4X4 model =  Matrix4X4().TRS(EntityAttachTo.transform.GetWordlPos(), EntityAttachTo.transform.rotate, Size);
...
{
	m_Model = entityAttach.scene->ressourcesManagers->GetElement<Model>("cube.obj");
