...
Model::Model(const fs::path& FilePath)
{
    ...
    const aiScene* scene = importer.ReadFile(pathToString, aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_FlipUVs | aiProcess_CalcTangentSpace);
    // check for errors
    if (!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !scene->mRootNode) // if is Not Zero
    {
        LOG("ERROR::ASSIMP:: " << importer.GetErrorString(),STATELOG::CRITICALERROR ) ;
        return;
    }
    // retrieve the directory path of the filepath
    directory = pathToString.substr(0, pathToString.find_last_of('/'));
...
std::vector<Texture*> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
{
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
    {
        ...
        {   // if texture hasn't been loaded already, load it
            ...
            ResourcesManager::PushBackElement<Texture>(directory,texture);
